 ...
 *
 * Ported byte-for-byte from the Android app's `listentogether/Protocol.kt` - this is the one file in
 * the whole feature where "identical" is not a nicety but the point: a phone and a desktop are meant
 * to be able to host and follow each other, and that only works if [VERSION] and every frame layout
 * match exactly. Do not change this file without changing that one the same way.
 *
 ...
 *
 * **Times are a monotonic clock in microseconds, never wall clock.** `System.currentTimeMillis()`
 * steps when NTP corrects it or the user edits the clock, and a step mid-session would look exactly
 * like a huge sync error and provoke a seek. `System.nanoTime()` is what this side uses; the phone
 * uses `SystemClock.elapsedRealtimeNanos()`. Neither side needs the other's epoch - the whole point
 * of the PING/PONG exchange is to measure the *offset* between two clocks that were never meant to
